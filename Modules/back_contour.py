...
def _det_intersect(self, line_endpoints, contour_pts):
...
#################################################################################################
    ...
    thirds = np.linspace(0, 1, num=4)[1:3]
    det_points = [self.hip + (np.array(self.shoulder) - np.array(self.hip))*t for t in thirds]
    # annotate det_points
    for d in det_points:
        cv2.circle(
            img=img,
...
###################################################################################################
    ...
    _write_contour_text(img, avg)

def _write_contour_text(self, img, avg):
    cur_eval = "straight"
    cur_color = colors.light_aqua
    if (avg == "None"):
        cur_eval = "none"
